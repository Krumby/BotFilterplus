            ...
            while ( sleep( 1000 ) )
            {
                try
                {
                    ...
                    for ( Map.Entry<String, Connector> entryset : bungee.getBotFilter().getConnectedUsersSet().entrySet() )
                    {
                        ...
                        BFPlus.CheckState state = connector.getState();
                        switch ( state )
                        {
                            case SUCCESSFULLY:
                            case FAILED:
                            ...
                            default:
                                ...
                                {
                                    connector.failed( PacketUtils.KickType.TIMED_OUT, state == BFPlus.CheckState.CAPTCHA_ON_POSITION_FAILED
                                            ? "Too long fall check" : "Captcha not entered" );
                                    TO_REMOVE_SET.add( entryset.getKey() );
                                    continue;
                                } else if ( state == BFPlus.CheckState.CAPTCHA_ON_POSITION_FAILED || state == BFPlus.CheckState.ONLY_POSITION )
                                {
                ...
                {
                    bungee.getLogger().log( Level.WARNING, "[BFPlus] Непонятная ошибка. Пожалуйста отправте ёё разработчику!", e );
                } finally
                {
        ...
        Thread t = new Thread( () ->
        {
            ...
            while ( !Thread.interrupted() && sleep( 5 * 1000 ) )
            {
                if ( ++counter == 12 )
                {
                    ...
                    {
                        BFPlus botFilter = bungee.getBotFilter();
