...
import ru.krumby.bfplus.captcha.BFPGenerator;
import ru.krumby.bfplus.config.Settings;
import ru.krumby.bfplus.utils.GeoIp;
import ru.krumby.bfplus.utils.ManyChecksUtils;
import ru.krumby.bfplus.utils.ServerPingUtils;
import ru.krumby.bfplus.utils.Sql;
...
public class BFPlus
{
    ...
    //UserName, Ip
    @Getter
    private final Map<String, BFPUser> userCache = new ConcurrentHashMap<>();
    ...
    public BFPlus(boolean startup)
    {
        Settings.IMP.reload( new File( "BFPlus", "config.yml" ) );
        Scoreboard.DISABLE_DUBLICATE = Settings.IMP.FIX_SCOREBOARD_TEAMS;
        checkForUpdates( startup );
        PacketUtils.init();
        if ( !CachedCaptcha.generated )
        {
            BFPGenerator.generateImages();
        }
    ...
    public void disable()
    {
        ...
        for ( Connector connector : connectedUsersSet.values() )
        {
            ...
            {
                connector.getUserConnection().disconnect( "§b§lBot§d§lFilter§b§l+§e Перезагрузка фильтра!" );
            }
    ...
    public void saveUser(String userName, InetAddress address, boolean afterCheck)
    {
        ...
        BFPUser BFPUser = userCache.get( userName );
        if ( BFPUser == null )
        {
            BFPUser = new BFPUser( userName, address.getHostAddress(), timestamp, timestamp );
        } else
        {
            BFPUser.setIp( address.getHostAddress() );
            BFPUser.setLastJoin( timestamp );
            if ( afterCheck )
            {
                BFPUser.setLastCheck( timestamp );
            }
        }

        userCache.put( userName, BFPUser);
        if ( sql != null )
        {
            sql.saveUser(BFPUser);
        }
    }

    public void addUserToCache(BFPUser BFPUser)
    {
        userCache.put( BFPUser.getName(), BFPUser);
    }
    ...
    {
        BFPUser BFPUser = userCache.get( userName.toLowerCase() );
        if ( Settings.IMP.PROTECTION.CHECK_LOCALHOST >= 1 && address.isLoopbackAddress() )
        {
            return Settings.IMP.PROTECTION.CHECK_LOCALHOST != 1;
        }
        return BFPUser == null || ( Settings.IMP.FORCE_CHECK_ON_ATTACK && isUnderAttack() )
            || !BFPUser.getIp().equalsIgnoreCase( address.getHostAddress() );
    }
    ...
    public void checkAsyncIfNeeded(InitialHandler handler)
    {
        ...
        if ( !Settings.IMP.PROTECTION.ALWAYS_CHECK && ManyChecksUtils.isManyChecks( address ) )
        {
            ...
            bungee.getLogger().log( Level.INFO, "(BFPlus) [{0}] disconnected: Too many checks in 10 min", address );
            return;
        }
        ...
        if ( ping.needCheck() && ping.needKickOrRemove( address ) )
        {
            ...
            bungee.getLogger().log( Level.INFO, "(BFPlus) [{0}] disconnected: The player did not ping the server", address.getHostAddress() );
            return;
        }
        ...
        if ( isGeoIpEnabled() )
        {
            executor.execute( () ->
            {
                if ( checkGeoIp( address ) )
                {
                    ...
                    bungee.getLogger().log( Level.INFO, "(BFPlus) [{0}] disconnected: Country is not allowed",
    ...
    private void checkForUpdates(boolean startup)
    {
        ...
        try
        {
            logger.log( Level.INFO, "§b§lBot§d§lFilter§b§l+§f Проверяю наличие обновлений" );
            URL url = new URL("https://raw.githubusercontent.com/PchelaMine/verr/main/version.txt");
                    ...
                    new InputStreamReader( conn.getInputStream() ) ) )
            {
                if ( !in.readLine().trim().equalsIgnoreCase( Settings.IMP.BOT_FILTER_VERSION ) )
                {
                    ...
                    logger.log(Level.INFO, "§b§lBot§d§lFilter§b§l+§f §aНайдена новая версия! §7(Дев-Билд?)");
                    logger.log(Level.INFO, "§b§lBot§d§lFilter§b§l+§f §aПожалуйста, обновитесь! §7(Дев-Билд?)");
                    logger.log(Level.INFO, "§b§lBot§d§lFilter§b§l+§f §ahttps://dsc.gg/bfplus");
                    if ( startup )
                    {
                        Thread.sleep( 3500L );
                    }
                ...
                {
                    logger.log( Level.INFO, "§b§lBot§d§lFilter§b§l+§f Обновлений не найдено!" );
                }
            }
        } catch ( IOException | InterruptedException ex )
        {
            logger.log( Level.WARNING, "§b§lBot§d§lFilter§b§l+§f Не могу проверить обновление", ex );
        }
    }
